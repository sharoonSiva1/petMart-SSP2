                ...
                <div class="border-b border-gray-200 mb-6">
                    <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                        <button wire:click="switchTab('products')"
                            class="{{ $activeTab === 'products' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">
                            Manage Products
                        </button>
                        <button wire:click="switchTab('orders')"
                            class="{{ $activeTab === 'orders' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center">
                            Manage Orders
                        ...
                        <button wire:click="switchTab('inquiries')"
                            class="{{ $activeTab === 'inquiries' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center">
                            View Complains
                                    ...
                                    @foreach($products as $product)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">Rs
                                                {{ number_format($product->price * 300, 2) }}
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 capitalize">
                                                {{ $product->pet_type }} - {{ str_replace('_', ' ', $product->category) }}
                                            </td>
                                    ...
                                    @foreach($inquiries as $inquiry)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                ...
                                                <div class="text-xs">{{ $inquiry->phone }}</div>
                                            </td>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        @error('product_category') <span class="text-red-500 text-xs">{{ $message }}</span>
                                        @enderror
                                    </div>
                                ...
                                <div>
                                    ...
                                    @elseif($existingImage)
                                        <img src="{{ $existingImage }}" class="mt-2 h-20 w-20 object-cover rounded">
                                    @endif
            ...
            <div class="p-6">
                <div class="mb-6">
                    ...
                    <div class="mt-2 text-sm text-gray-600 space-y-1">
                        <p>Placed on: {{ $selectedOrder->created_at->format('M d, Y H:i') }}</p>
                        <p>Status: <span class="font-medium text-gray-900">{{ ucfirst($selectedOrder->status) }}</span></p>
                    </div>
                </div>
                ...
                <div class="mb-6 bg-gray-50 p-4 rounded-md border border-gray-100">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            <p class="font-medium text-gray-900">{{ $selectedOrder->user->name ?? $selectedOrder->name }}</p>
                        ...
                        <div>
                            ...
                            <p class="font-medium text-gray-900">{{ $selectedOrder->address }}</p>
                            <p class="font-medium text-gray-900">{{ $selectedOrder->city }}</p>
                        </div>
                    </div>
                </div>
                                    ...
                                    <div class="flex justify-between">
                                        <div>
                                            <h5 class="text-sm font-medium text-gray-900">
                                                {{ $item->product->name ?? 'Unknown Product' }}</h5>
                                            <p class="mt-1 text-xs text-gray-500">Qty: {{ $item->quantity }}</p>
                                        </div>
                                        <p class="text-sm font-medium text-gray-900">Rs {{ number_format($item->price, 2) }}</p>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                ...
                <div class="border-t border-gray-200 pt-4 mt-4 flex justify-between items-center">
                    ...
                    <div class="text-lg font-bold text-orange-600">Rs {{ number_format($selectedOrder->total, 2) }}</div>
                </div>
