            ...
            <div
                ...
                <div class="mt-3 flex flex-col items-center text-center">
                    ...
                    <div class="mt-2 flex flex-col items-center">
                        <p class="text-sm font-bold text-gray-900">Rs {{ number_format($product->price * 300, 2) }} LKR</p>
                        <!-- Converting approximate USD to LKR for visual -->
                        @if($loop->index % 3 == 0)
                            <p class="text-xs text-red-500 line-through">Rs
                                {{ number_format(($product->price * 300) * 1.2, 2) }}
                            </p>
                        @endif
                    </div>
                        ...
                        @unless(auth()->user()?->is_admin)
                            <div x-data="{ added: false }"
                                ...
                                <button wire:click.prevent="addToCart({{ $product->id }})" wire:loading.attr="disabled"
                                    ...
                                    <!-- Default State -->
                                    <span wire:loading.remove wire:target="addToCart({{ $product->id }})" x-show="!added">
                                        Add to cart
                                    </span>
